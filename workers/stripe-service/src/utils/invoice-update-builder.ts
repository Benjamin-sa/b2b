 ...
 */
export function addInvoiceBasicFields(builder: UpdateBuilder, invoice: Stripe.Invoice): void {
 ...
 */
export function extractShippingCost(invoice: Stripe.Invoice): [number, string | null] {
  if (!invoice.lines?.data) {
    return [0, null];
  }

  const shippingItem = invoice.lines.data.find((line) => line.metadata?.type === 'shipping');
