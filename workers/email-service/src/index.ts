  ...
  async queue(batch: MessageBatch<EmailQueueMessage>, env: Environment): Promise<void> {
    ...
    for (const message of batch.messages) {
      try {
        ...
        console.log(
          `📧 [Email Queue] Processing ${emailMessage.type} email for ${emailMessage.email}`
        );
        ...
        if (result.success) {
          console.log(
            `✅ [Email Queue] ${emailMessage.type} email sent successfully to ${emailMessage.email}`
          );
          message.ack(); // Success - remove from queue
        } else {
          console.error(
            `❌ [Email Queue] Failed to send ${emailMessage.type} email:`,
            result.error
          );
