...
adminOrchestration.get('/users/:userId', async (c) => {
  try {
    ...
    if (!response.ok) {
      console.error('❌ [Gateway] Failed to fetch user details:', response.status);
      return response;
    }
...
adminOrchestration.post('/users/:userId/verify', async (c) => {
  try {
    ...
    const authRequest = new Request(`http://internal/admin/users/${userId}/verify`, {
      method: 'POST',
      headers,
    });
    ...
    if (!authResponse.ok) {
      console.error('❌ [Gateway] User verification failed:', authResponse.status);
      return authResponse;
    }
    ...
    console.log('📧 [Gateway] Queuing verification email for:', verificationResult.user.email);
    ...
    try {
      ...
      console.error('⚠️  [Gateway] Failed to queue verification email:', emailError);
    }
...
adminOrchestration.post('/users/:userId/reset-password', async (c) => {
  try {
    ...
    const request = new Request(`http://internal/admin/users/${userId}/reset-password`, {
      method: 'POST',
      headers,
      body: clonedRequest.body,
    });
