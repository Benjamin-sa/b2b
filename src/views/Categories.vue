<template>
    <div class="min-h-screen bg-gradient-to-br from-gray-50 to-primary-50/20">
        ...
        <div class="bg-white shadow-sm border-b-2 border-primary-100">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-0">
                    <div>
                        <h1 class="text-2xl sm:text-3xl lg:text-4xl font-bold bg-gradient-to-r from-primary-600 to-primary-700 bg-clip-text text-transparent">{{ $t('categories.title') }}</h1>
                        <p class="mt-1 sm:mt-2 text-sm sm:text-base text-gray-600">{{ $t('categories.subtitle') }}</p>
                    </div>
                    <div class="text-xs sm:text-sm text-primary-600 font-medium bg-primary-50 px-3 py-1.5 rounded-full w-fit">
        ...
        <!-- Loading State -->
        <div v-if="categoryStore.isLoading && !categoryStore.hasCategories" class="text-center py-16 sm:py-24">
            <div class="w-14 h-14 sm:w-16 sm:h-16 border-4 border-primary-200 border-t-primary-600 rounded-full animate-spin mx-auto"></div>
            <p class="mt-4 sm:mt-6 text-base sm:text-lg text-gray-600 font-medium">{{ $t('categories.loading') }}</p>
        </div>

        <!-- Error State -->
        <div v-else-if="categoryStore.error" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
            <div class="bg-danger-50 border-2 border-danger-200 rounded-lg sm:rounded-xl p-4 sm:p-6">
                <div class="flex flex-col sm:flex-row items-start gap-3 sm:gap-0">
                    <svg class="w-5 h-5 sm:w-6 sm:h-6 text-danger-500 sm:mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <div class="flex-1">
                        <h3 class="text-base sm:text-lg font-medium text-danger-800">{{ $t('categories.error') }}</h3>
        ...
        <div v-else class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
            <div v-if="rootCategories.length === 0" class="text-center py-16 sm:py-24">
                ...
                <h3 class="mt-4 text-base sm:text-lg font-medium text-gray-900">{{ $t('categories.noCategories') }}</h3>
                <p class="mt-2 text-sm sm:text-base text-gray-500">{{ $t('categories.noCategoriesMessage') }}</p>
            </div>
