      ...
      <div
        ...
        <div>
          <h1 class="text-2xl sm:text-3xl lg:text-4xl font-black text-gray-900 flex items-center">
            <span
              class="w-1.5 sm:w-2 h-8 sm:h-10 bg-gradient-to-b from-primary-600 to-primary-700 rounded-full mr-2 sm:mr-4"></span>
            {{ $t('orders.title') }}
          </h1>
            ...
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0">
              <div class="w-full sm:w-auto">
                <h3 class="text-base sm:text-xl font-bold text-gray-900 flex items-center">
                  <svg class="w-4 h-4 sm:w-5 sm:h-5 mr-2 text-primary-600 flex-shrink-0" fill="none"
                    stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                      d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                  </svg>
                  <span class="truncate">{{ $t('orders.invoice') }}
                    {{ order.invoice_number || order.stripe_invoice_id || order.id }}</span>
                </h3>
                <p class="text-xs sm:text-sm text-gray-700 mt-1 ml-6 sm:ml-7 font-medium">
                  {{ $t('orders.orderPlacedOn') }} {{ formatDate(order.order_date) }}
                </p>
              </div>
              <div class="flex items-center space-x-3 sm:space-x-4 w-full sm:w-auto justify-between sm:justify-end">
                <!-- Status Badge -->
                <span :class="getStatusBadgeClass(order.stripe_status || order.status)"
                  ...
                  {{ getStatusText(order.stripe_status || order.status) }}
                </span>
                <!-- Amount -->
                <div class="text-right">
                  ...
                  <p v-if="order.stripe_status === 'paid'" class="text-xs sm:text-sm text-success-700 font-bold">
                    {{ $t('orders.paidOn') }} {{ formatDate(order.paid_at || order.updated_at) }}
                  </p>
                  <p v-else-if="order.due_date" class="text-xs sm:text-sm text-gray-700 font-medium">
                    {{ $t('orders.dueOn') }} {{ formatDate(order.due_date) }}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div class="px-4 sm:px-6 py-4 space-y-4 sm:space-y-6">
            ...
            <div v-if="order.items && order.items.length > 0">
              <h4 class="text-sm sm:text-base font-bold text-gray-900 mb-3 sm:mb-4 flex items-center">
                <span
                  class="w-1 h-5 sm:h-6 bg-gradient-to-b from-secondary-600 to-secondary-700 rounded-full mr-2"></span>
                {{ $t('orders.items') }}
              </h4>
          ...
          <div
            ...
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 sm:gap-0">
              <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3">
                ...
                <!-- Download PDF -->
                <a v-if="order.invoice_pdf" :href="order.invoice_pdf" target="_blank"
              ...
              <div class="text-sm text-gray-500">
                <span v-if="order.stripe_status === 'paid'">{{ $t('orders.paymentReceived') }}</span>
                <span v-else-if="order.stripe_status === 'sent'">{{
                  $t('orders.invoiceSent')
                }}</span>
                <span v-else-if="order.stripe_status === 'open'">{{
                  $t('orders.awaitingPayment')
                }}</span>
                <span v-else-if="order.stripe_status === 'draft'">{{
                  $t('orders.status.draft')
                }}</span>
                <span v-else>{{ getStatusText(order.stripe_status || order.status) }}</span>
              </div>
            </div>
          </div>
        </div>
