<template>
  <div class="min-h-screen bg-gray-50">
    <!-- Loading State -->
    <div v-if="isLoading"
      class="flex items-center justify-center min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 px-4">
    ...
    <div v-else-if="!product"
      class="flex items-center justify-center min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 px-4">
      <div
        class="text-center max-w-md mx-auto bg-white rounded-xl sm:rounded-2xl shadow-2xl p-6 sm:p-10 border border-gray-100">
        <div
          class="bg-gradient-to-br from-danger-100 to-danger-200 rounded-full w-16 h-16 sm:w-20 sm:h-20 mx-auto mb-4 sm:mb-6 flex items-center justify-center">
    ...
    <div v-else class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
      ...
      <nav class="flex mb-4 sm:mb-8 overflow-x-auto" aria-label="Breadcrumb">
        <ol class="flex items-center space-x-1 sm:space-x-2 text-xs sm:text-sm whitespace-nowrap">
          ...
          <li>
            <span class="text-gray-900 font-medium truncate max-w-[150px] sm:max-w-none">{{
              product.name
              }}</span>
          </li>
        </ol>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-12">
        <!-- Left Column - Images -->
        <div class="space-y-6">
          <ImageGallery :images="productImages" :alt="product.name" :show-thumbnails="true" :allow-zoom="true" />
        </div>
        ...
        <div class="space-y-4 sm:space-y-6 lg:space-y-8">
          ...
          <div>
            <div class="flex flex-col sm:flex-row items-start justify-between mb-3 sm:mb-4 gap-3">
              <div class="flex-1 w-full sm:w-auto">
                ...
                <div class="flex flex-wrap items-center gap-2 sm:gap-4 text-xs sm:text-sm text-gray-600">
                  ...
                  <span v-if="product.barcode" class="font-mono text-gray-800">
                    {{ $t('productDetail.barcode', { barcode: product.barcode }) }}
                  </span>
                  <span v-if="product.part_number">{{
            ...
            <!-- Price -->
            <div
              class="mb-4 sm:mb-6 lg:mb-8 bg-gradient-to-br from-primary-50 to-primary-100 rounded-xl sm:rounded-2xl p-4 sm:p-6 border-2 border-primary-200 shadow-md">
              <div class="flex flex-wrap items-baseline gap-2 sm:gap-3 mb-2">
                <span
                  class="text-3xl sm:text-4xl font-black bg-gradient-to-r from-primary-600 to-primary-700 bg-clip-text text-transparent">
              ...
              <p class="text-sm sm:text-base text-gray-700 font-medium">
                {{
                  $t('productDetail.per', {
                    unit: product.unit || $t('productDetail.piece'),
                  })
                }}
              </p>
            </div>
          ...
          <!-- Purchase Section -->
          <div
            class="bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-8 border border-gray-200 sm:border-2 shadow-lg">
            <h3 class="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 mb-4 sm:mb-6 flex items-center">
              <span
                class="w-1 sm:w-1.5 h-6 sm:h-8 bg-gradient-to-b from-primary-600 to-primary-700 rounded-full mr-2 sm:mr-3"></span>
            ...
            <div class="mb-4 sm:mb-6 lg:mb-8">
              ...
              <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4">
                <div
                  class="flex items-center border border-primary-300 sm:border-2 rounded-lg sm:rounded-xl bg-white shadow-md hover:shadow-lg transition-shadow">
                  <button :disabled="!canAddMore || Boolean(product.coming_soon) || quantity <= inputMinValue
                    "
                  ...
                  <input v-model.number="quantity" type="number" :min="inputMinValue" :max="inputMaxValue"
                    :disabled="!canAddMore || Boolean(product.coming_soon)"
                  ...
                  <button :disabled="!canAddMore || Boolean(product.coming_soon) || quantity >= inputMaxValue
                    "
                ...
                <div class="text-xs sm:text-sm text-gray-600">
                  <div v-if="product.min_order_quantity">
                    {{ $t('productDetail.min') }} {{ product.min_order_quantity }}
                    {{ product.unit || $t('productDetail.pieces') }}
                  </div>
                  <div v-if="isLimitedByStock">
                    {{ $t('productDetail.max') }} {{ inputMaxValue }}
                    {{ product.unit || $t('productDetail.pieces') }}
                    <span class="text-red-600">{{ $t('productDetail.limitedByStock') }}</span>
                  </div>
                  <div v-else-if="product.max_order_quantity">
                    {{ $t('productDetail.max') }} {{ product.max_order_quantity }}
                    {{ product.unit || $t('productDetail.pieces') }}
                  </div>
            ...
            <!-- Total Price -->
            <div
              class="mb-4 sm:mb-6 lg:mb-8 p-4 sm:p-6 bg-gradient-to-br from-primary-50 to-primary-100 rounded-lg sm:rounded-xl border border-primary-300 sm:border-2 shadow-md">
              <div class="flex flex-col sm:flex-row justify-between items-center gap-2 sm:gap-0">
                ...
                <span
                  class="text-2xl sm:text-3xl lg:text-4xl font-black bg-gradient-to-r from-primary-600 to-primary-700 bg-clip-text text-transparent">€{{
                    formatPrice(product.price * quantity) }}</span>
              </div>
            </div>

            <!-- Add to Cart Button -->
            <button :disabled="Boolean(product.coming_soon) ||
              ...
              <!-- Loading spinner -->
              <svg v-if="isAddingToCart" class="animate-spin -ml-1 mr-2 sm:mr-3 h-4 w-4 sm:h-5 sm:w-5 text-white"
              ...
              <!-- Success checkmark -->
              <svg v-else-if="addedToCartRecently" class="w-4 h-4 sm:w-5 sm:h-5 mr-1.5 sm:mr-2" fill="none"
              ...
              <span v-else-if="!canOrder">{{
                $t('productDetail.accountVerificationRequired')
                }}</span>
            ...
            <div v-if="addedToCartRecently" class="mt-3 sm:mt-4 grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
              ...
              <router-link to="/checkout"
                ...
                <svg class="w-4 h-4 sm:w-5 sm:h-5 ml-1.5 sm:ml-2 flex-shrink-0" fill="none" stroke="currentColor"
                  viewBox="0 0 24 24">
...
const canAddMore = computed(() => {
  return (
    !!product.value &&
    !product.value.coming_soon &&
    product.value.in_stock &&
...
watch(
  () => [
    product.value ? product.value.id : null,
    minSelectableQuantity.value,
