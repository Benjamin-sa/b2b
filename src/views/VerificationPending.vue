    ...
    <div class="max-w-md w-full bg-white rounded-lg shadow-xl p-8 text-center">
      ...
      <div class="bg-gray-50 rounded-lg p-4 mb-6 text-left">
        <div class="mb-2">
          <span class="text-sm font-medium text-gray-700">{{
            $t('verificationPending.company')
          }}</span>
          <span class="text-sm text-gray-900 ml-2">{{ userProfile?.company_name }}</span>
        </div>
        <div class="mb-2">
          <span class="text-sm font-medium text-gray-700">{{
            $t('verificationPending.email')
          }}</span>
          <span class="text-sm text-gray-900 ml-2">{{ userProfile?.email }}</span>
        </div>
        <div class="mb-2">
          <span class="text-sm font-medium text-gray-700">{{
            $t('verificationPending.btwNumber')
          }}</span>
          <span class="text-sm text-gray-900 ml-2">{{ userProfile?.btw_number }}</span>
        </div>
      </div>
      ...
      <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
        <h2 class="text-lg font-semibold text-blue-900 mb-2">
          {{ $t('verificationPending.whatHappensNext') }}
        </h2>
        <p class="text-blue-800 text-sm" v-html="$t('verificationPending.whatHappensNextMessage')"></p>
      </div>
      <!-- Action Buttons -->
      <div class="space-y-3">
        <button
