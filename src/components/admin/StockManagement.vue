<template>
  <div class="max-w-[1400px] mx-auto p-8">
          ...
          <tr v-for="product in filteredProducts" :key="product.id"
            ...
            <td class="px-4 py-4 text-center">
              <button
                ...
                title="Edit stock" @click="openEditModal(product)">
    ...
    <!-- Edit Stock Modal -->
    <div v-if="editingProduct" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4"
      ...
      <div class="bg-white rounded-xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        <div class="flex justify-between items-center px-6 py-6 border-b border-gray-200">
          ...
          <button
            class="flex items-center justify-center w-8 h-8 bg-transparent text-gray-500 rounded-md transition-all hover:bg-gray-100"
        ...
        <div class="px-6 py-6 overflow-y-auto">
          ...
          <div class="flex items-center gap-4 p-4 bg-gray-50 rounded-lg mb-6">
            <img v-if="editingProduct.image_url" :src="editingProduct.image_url" :alt="editingProduct.name"
              class="w-16 h-16 rounded-lg object-cover" />
            <div>
              <h4 class="text-lg font-semibold text-gray-900">{{ editingProduct.name }}</h4>
              <p v-if="editingProduct.part_number" class="text-sm text-gray-500">
          ...
          <!-- Update Form -->
          <form class="flex flex-col gap-4" @submit.prevent="updateStock">
            <!-- Stock Amount -->
            <div class="flex flex-col gap-2">
              <label class="text-sm font-medium text-gray-700">Available Stock</label>
              <input v-model.number="stockUpdate.totalStock" type="number" min="0"
                class="px-2.5 py-2.5 border border-gray-300 rounded-md text-sm transition-all focus:outline-none focus:border-blue-500 focus:ring-3 focus:ring-blue-100"
                required />
              <p class="text-xs text-gray-500">
                Stock is shared between B2B and Shopify. Shopify is the source of truth.
              </p>
            </div>
            ...
            <div class="flex gap-4 justify-end mt-6 pt-6 border-t border-gray-200">
              <button type="button"
                class="px-5 py-2.5 bg-white text-gray-700 border border-gray-300 rounded-lg text-sm font-medium hover:bg-gray-50 transition-colors"
                @click="closeEditModal">
                Cancel
              </button>
              <button type="submit"
...
const stockUpdate = ref<StockUpdate>({
  ...
  stockMode: 'unified', // Always unified now
});
...
const validationError = computed(() => {
  ...
  if (stockUpdate.value.totalStock < 0) {
...
const unallocatedStock = computed(() => 0);
...
// SIMPLIFIED: Always returns 'unified' now
const getStockMode = () => 'unified';
...
const openEditModal = (product: ProductWithInventory) => {
  ...
  stockUpdate.value = {
    ...
    b2bStock: currentStock, // Same as total in unified mode
    b2cStock: 0,
...
const updateStock = async () => {
  ...
  try {
    const response = await fetch(
      `${API_GATEWAY_URL}/api/products/inventory/${editingProduct.value.id}/stock`,
      {
        method: 'POST',
        headers: {
