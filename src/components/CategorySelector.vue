      ...
      <div class="py-3 sm:py-4">
          ...
          <template v-for="category in categoryStore.rootCategories" :key="category.id">
            ...
            >
              <span
                ...
                class="w-2 h-2 sm:w-3 sm:h-3 rounded-full inline-block mr-1 sm:mr-2 shadow-sm"
              ></span>
              {{ category.name }}
            </button>
              ...
              <div class="flex gap-1 sm:gap-1.5">
                ...
                >
                  <span
                    ...
                    class="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full inline-block mr-1 shadow-sm"
                  ></span>
                  {{ child.name }}
                </button>
              </div>
        ...
        >
          <template v-for="(item, index) in breadcrumb" :key="index">
            ...
            <svg
              v-if="index < breadcrumb.length - 1"
              class="w-3 h-3 sm:w-4 sm:h-4 text-primary-400 flex-shrink-0"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
        ...
        <div v-if="selectedCategory?.description" class="mt-2">
          <p
            class="text-xs sm:text-sm text-gray-600 bg-primary-50 px-3 py-2 rounded-lg border border-primary-100"
          >
