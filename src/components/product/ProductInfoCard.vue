            ...
            <nav class="flex">
                <button v-for="tab in tabs" :key="tab.id" @click="activeTab = tab.id" :class="[
                    ...
                    <div class="flex items-center justify-center">
                        <svg v-if="tab.id === 'specifications'" class="w-3 h-3 sm:w-4 sm:h-4 mr-1 sm:mr-2" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                        </svg>
                        <svg v-else class="w-3 h-3 sm:w-4 sm:h-4 mr-1 sm:mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div v-if="activeTab === 'specifications'" key="specifications">
                    <div v-if="specifications && specifications.length > 0" class="space-y-1">
                        <div v-for="(spec, index) in specifications" :key="index"
                            ...
                            <span class="text-xs sm:text-sm font-medium text-gray-600 col-span-1">{{ spec.key }}</span>
