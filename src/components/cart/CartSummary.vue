  ...
  <div
    ...
    <div class="space-y-2">
      ...
      <div class="flex justify-between text-xs sm:text-sm">
        ...
        <span :class="shipping_cost === 0 ? 'text-success-600 font-bold' : 'text-gray-900 font-medium'">
          <template v-if="shipping_cost === 0">
            {{ $t('cart.free') }}
          </template>
          <template v-else> €{{ formatPrice(shipping_cost) }} </template>
        </span>
      </div>

      <!-- Tax (only for Belgian customers) -->
      <div v-if="should_show_vat" class="flex justify-between text-xs sm:text-sm">
      ...
      <div class="flex justify-between text-sm sm:text-base font-bold pt-1">
        ...
        <span class="text-primary-700"> €{{ formatPrice(grand_total) }} </span>
      </div>

      <!-- VAT Info (only for Belgian customers) -->
      <p v-if="should_show_vat" class="text-xs text-gray-500 mt-2">
        {{ $t('cart.vatInfo') }}
      </p>
    ...
    <div class="mt-4 sm:mt-6 space-y-2 sm:space-y-3">
      <!-- Checkout Button -->
      <button :disabled="item_count === 0 || isLoading"
      ...
      <!-- Clear Cart -->
      <button v-if="item_count > 0" :disabled="isLoading"
...
const item_count = computed(() => cartStore.item_count);
const subtotal = computed(() => cartStore.subtotal);
const shipping_cost = computed(() => cartStore.shipping_cost);
const tax = computed(() => cartStore.tax);
const grand_total = computed(() => cartStore.grand_total);
const should_show_vat = computed(() => cartStore.should_show_vat);
