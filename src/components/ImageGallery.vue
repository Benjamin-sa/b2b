<template>
  <div class="space-y-2 sm:space-y-3 lg:space-y-4">
    <!-- Main Image Display -->
    <div
      class="relative aspect-square bg-gray-100 rounded-lg sm:rounded-xl overflow-hidden shadow-md sm:shadow-lg"
    >
      <img
        v-if="currentImage"
        :src="currentImage"
        :alt="alt || $t('imageGallery.productImage')"
    ...
    <div
      ...
      class="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 gap-1.5 sm:gap-2"
    >
      <button
        v-for="(image, index) in images"
        :key="index"
        class="aspect-square rounded-md sm:rounded-lg overflow-hidden border transition-all sm:border-2"
        :class="[
      ...
      >
        <img
          :src="image"
          :alt="$t('imageGallery.thumbnail', { index: index + 1 })"
          class="w-full h-full object-cover"
          @error="handleImageError"
        />
      </button>
